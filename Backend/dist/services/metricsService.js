...
async function buildMetrics() {
    // --- Summary values
    const totalRes = await (0, dbService_1.query)(`SELECT count(*) FROM public.workflows`);
    const totalDocuments = Number(totalRes.rows[0]?.count || 0);
    const autoRes = await (0, dbService_1.query)(`SELECT count(*) FROM public.workflows WHERE status = 'AI_AUTO_APPROVED'`);
    const autoApproved = Number(autoRes.rows[0]?.count || 0);
    const rejectedRes = await (0, dbService_1.query)(`SELECT count(*) FROM public.workflows WHERE status = 'REJECTED'`);
    const rejectedCount = Number(rejectedRes.rows[0]?.count || 0);
    // pending review = in AI_PROCESSED or UNDER_REVIEW
    const pendingRes = await (0, dbService_1.query)(`SELECT count(*) FROM public.workflows WHERE status IN ('AI_PROCESSED','UNDER_REVIEW')`);
    ...
    const avgRes = await (0, dbService_1.query)(`
    ...
    const monthlyTrendRes = await (0, dbService_1.query)(`
      ...
      `);
    const monthlyTrend = monthlyTrendRes.rows.map((r) => {
    ...
    // --- By Type
    const byTypeRes = await (0, dbService_1.query)(`
      SELECT COALESCE(type,'unknown') as type, count(*) as count
      FROM public.workflows
      GROUP BY 1
      ORDER BY count DESC
    `);
    const byType = byTypeRes.rows.map((r) => ({ type: r.type, count: Number(r.count || 0) }));
    // --- Turnaround distribution buckets (hours between submitted -> first approved)
    const turnaroundRes = await (0, dbService_1.query)(`
      ...
      ORDER BY
        CASE WHEN bucket = '<1h' THEN 1
             ...
             ELSE 99 END
    `);
    const turnaroundDistribution = turnaroundRes.rows.map((r) => ({ range: r.bucket, count: Number(r.count || 0) }));
    // --- Confidence distribution buckets (ai_result->>'confidence')
    const confidenceRes = await (0, dbService_1.query)(`
      SELECT bucket, count FROM (
        SELECT
          CASE
    ...
    `);
    const confidenceDistribution = confidenceRes.rows.map((r) => ({ range: r.bucket, count: Number(r.count || 0) }));
    return {
        summary: {
            totalDocuments,
            autoApprovalRate,
